...
//    glTF convention (Y-up, metres): rotation -90 deg about X, uniform
//    scale 0.001. Everything below it — node translations, animation
//    outputs, mesh vertices — stays in chain millimetres, Z-up.
...
//    prismatic joints and the root as translation samplers. Channels that
//    stay at rest through the whole clip are skipped. Consecutive
//    quaternions are sign-aligned so naive component lerp never spins the
//    long way.
//  - loop.mode pingpong bakes the way back (times mirrored past the last
//    keyframe) so one playthrough is one seamless loop period; hold and
//    cycle export the authored keys. The loop block itself also rides in
//    asset "extras" for consumers that want to do better.
